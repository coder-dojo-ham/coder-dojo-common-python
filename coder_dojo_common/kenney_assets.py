...
def get_assets(session: Session, base_url: str) -> KenneyAssetsModel:
    ...
    while True:
        ...
        asset_urls = [div.a["href"] for div in soup.find_all(id="content")[0].children if
                      isinstance(div, bs4.element.Tag)]
        found_assets.extend([AssetModel(name=url.split("/")[-1], url=url) for url in asset_urls])
        page_number += 1
